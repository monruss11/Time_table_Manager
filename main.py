...
@app.post('/student/manage',response_class=HTMLResponse)
async def res(request: Request, operation:Optional[str]=Form(None)):
  global check_count; global str_url; global row_count; global col_count

  data_page={ 'title':operation }
  ...
  if operation=="Edit Student":
    data_page={'title':'Edit Student','action':'Edit'}
    return templates.TemplateResponse('edit_stud_page.html',\
                        {'request': request, 'data_page': data_page})
...
def new_student(request: Request, std_name:Optional[str] = Form(None),std_family:Optional[str]= Form(None),\
  ...
  student=Student(std_name,std_family,std_nickname); db.add(student); db.commit()
